...
public interface HoadonBHRepository extends JpaRepository<hoadonbanhang, Integer>{
	...
	//@Query("SELECT e FROM hoadonbanhang e WHERE e.mahoadon = :mahoadon")
	  ...
	  @Query(value = "SELECT * FROM hoadonbanhang  INNER JOIN users ON hoadonbanhang.nguoitao=users.manhanvien  WHERE makhachhang=:makhachhang AND (hoadonbanhang.tonggia-hoadonbanhang.khachhangtra - (SELECT SUM(`sotienthu`) FROM `phieuthu` WHERE `idhoadon` = hoadonbanhang.id))>0    ORDER BY hoadonbanhang.created_at DESC",
			  countQuery = "SELECT COUNT(*) FROM (SELECT * FROM hoadonbanhang  INNER JOIN users ON hoadonbanhang.nguoitao=users.manhanvien  WHERE makhachhang=:makhachhang AND (hoadonbanhang.tonggia-hoadonbanhang.khachhangtra - (SELECT SUM(`sotienthu`) FROM `phieuthu` WHERE `idhoadon` = hoadonbanhang.id))>0  ORDER BY hoadonbanhang.created_at DESC) as temp ",
			  nativeQuery = true)
